...
var _runtime = require("regenerator-runtime/runtime"); //to polyfill everything else.
//importing model(state,loadRecipe()) from model.js (to update the View part/to display recipe)
var _modelJs = require("../js/model.js"); //to get state and to load recipes asynchronously 
var _recipeViewJs = require("./views/recipeView.js"); //to display DOM changes. 
var _recipeViewJsDefault = parcelHelpers.interopDefault(_recipeViewJs);
var _searchViewJs = require("./views/searchView.js"); //to get query and listen to `click` event. 
var _searchViewJsDefault = parcelHelpers.interopDefault(_searchViewJs);
var _resultsViewJs = require("./views/resultsView.js"); //to display results in LHS and to use methods 
var _resultsViewJsDefault = parcelHelpers.interopDefault(_resultsViewJs);
var _paginationViewJs = require("./views/paginationView.js"); //to display page buttons. 
var _paginationViewJsDefault = parcelHelpers.interopDefault(_paginationViewJs);
var _bookmarkViewJs = require("./views/bookmarkView.js"); //to display bookmarked recipes.
var _bookmarkViewJsDefault = parcelHelpers.interopDefault(_bookmarkViewJs);
var _addRecipeViewJs = require("./views/addRecipeView.js"); //to show and hide addRecipe window.
var _addRecipeViewJsDefault = parcelHelpers.interopDefault(_addRecipeViewJs);
var _configJs = require("./config.js"); //for MODAL_CLOSE_SEC global variable
//<-----------------------------------JS CODE------------------------------------------------->
...
init();
function init() {
    (0, _searchViewJsDefault.default).searchViewListener(controlSearchResults); //0.1-->to add eventListener to the search button
    (0, _recipeViewJsDefault.default).recipeViewListener(controlRecipes); //0.2-->to add event listener to the search results(ResultsView)and display particular recipe in RHS
    (0, _recipeViewJsDefault.default).servingsUpdateListener(controlServings); //0.3-->to listen when number of servings are updated.
    (0, _recipeViewJsDefault.default).addBookmarkListener(controlBookmark); //0.4-->to listen click event whenever user clicks on bookmark button 
    (0, _paginationViewJsDefault.default).pageBtnClickListener(controlPagination); //0.5-->to listen either previous page/next page button was clicked
    (0, _bookmarkViewJsDefault.default).addHandlerRender(controlRenderBMs); //0.6-->when page loads,render bookmarks
    (0, _addRecipeViewJsDefault.default).addHandlerUpload(controlAddRecipe); //0.7-->when user uploads recipe,receive data into controller--then to-->MODEL
}
...
async function controlRecipes() {
    try {
        ...
        (0, _recipeViewJsDefault.default).startSpinner();
        ...
        (0, _resultsViewJsDefault.default).update(_modelJs.state.searchResults.currentPageResults);
        ...
        let { recipe: recipeDetails } = _modelJs.state;
        //1.4-->after loading recipe details now display recipe on RHS(in .recipe div)(RECIPEVIEW)
        (0, _recipeViewJsDefault.default).render(recipeDetails); //you can also pass model.state.recipe
    } catch (error) {
        console.log(error);
        //1.5-->to stop spinner when we have encountered any error.
        (0, _recipeViewJsDefault.default).stopSpinner(); //(RECIPEVIEW)
        //1.6-->displaying error for users from VIEW file
        (0, _recipeViewJsDefault.default).displayError(); //(RECIPEVIEW)
    }
}
...
async function controlSearchResults() {
    try {
        ...
        const query = (0, _searchViewJsDefault.default).getQuery(); //(SEARCHVIEW)
        ...
        (0, _resultsViewJsDefault.default).startSpinner();
        //2.4-->load the searchResults and store it in model.state
        await _modelJs.loadSearchResults(query); //(SEARCHVIEW)
        //2.5-->stopSpinner after loading recipes in LHS(RESULTSVIEW) should i stop this after displaying the recipes in LHS.
        (0, _resultsViewJsDefault.default).stopSpinner();
        const totalRecipes = _modelJs.state.searchResults.totalResults; //if totalRecipes = 0,then that error is handled in model.js
        const totalPages = _modelJs.state.searchResults.totalPages;
        //displaySuccessMessage and display results on LHS
        (0, _recipeViewJsDefault.default).displaySuccessMessage(`Found ${totalRecipes} search results for this recipe which are displayed using ${totalPages} pages`);
        //This is for default page/first page.
        _modelJs.currentPageSearchResults();
        //2.6-->to display current page results in LHS(RECIPEVIEW)
        (0, _resultsViewJsDefault.default).render(_modelJs.state.searchResults.currentPageResults);
        //2.7-->to display page buttons for currentPage.(PaginationVIEW)
        (0, _paginationViewJsDefault.default).render(_modelJs.state.searchResults.currentPageNumber); //1 is default as soon as results load
    } catch (error) {
        console.log(error);
        //when query is empty,or no results found for query
        (0, _resultsViewJsDefault.default).displayError(error);
        (0, _recipeViewJsDefault.default).displayError(error);
        // 2.8-->to clear buttons when error occurs
        (0, _paginationViewJsDefault.default).handleError();
    }
}
//3-->to handle whenever pageButtons are clicked
function controlPagination(goToPage) {
    _modelJs.state.searchResults.currentPageNumber = goToPage;
    _modelJs.currentPageSearchResults();
    //2.6-->to display current page results in LHS(RECIPEVIEW)
    (0, _resultsViewJsDefault.default).render(_modelJs.state.searchResults.currentPageResults);
    //2.7-->to display page buttons for currentPage.(PaginationVIEW)
    (0, _paginationViewJsDefault.default).render(_modelJs.state.searchResults.currentPageNumber);
}
...
function controlServings(newServings) {
    ...
    let { recipe: recipeDetails } = _modelJs.state;
    (0, _recipeViewJsDefault.default).update(recipeDetails);
/*
...
function controlBookmark() {
    const curRecipe = _modelJs.state.recipe;
    //change status,change icon,update bookmarks[],update local storage
    //5.1-->change status
    _modelJs.toggleBookmarkStatus(curRecipe.isBookmarked); //MODEL
    //5.2-->change icon
    (0, _recipeViewJsDefault.default).update(curRecipe);
    //5.3-->update array
    _modelJs.updateBookmarksArray(curRecipe.isBookmarked); //MODEL
    //5.4-->display in bookmark button
    (0, _bookmarkViewJsDefault.default).render(_modelJs.state.bookmarks);
}
function controlRenderBMs() {
    let bookmarksArray = _modelJs.getLocalStorage();
    _modelJs.state.bookmarks = bookmarksArray;
    (0, _bookmarkViewJsDefault.default).render(_modelJs.state.bookmarks);
}
//<--------------------------------------E)CODE RELATED TO ADDING RECIPE BUTTON---------------------------------------------->
...
async function controlAddRecipe(newRecipe) {
    try {
        ...
        (0, _addRecipeViewJsDefault.default).startSpinner();
        ...
        (0, _addRecipeViewJsDefault.default).displaySuccessMessage();
        //6.6-->close the form
        setTimeout(function() {
        ...
        (0, _addRecipeViewJsDefault.default).displayError(error.message); //where is the error getting displayed.
    }
}

},{"core-js/modules/web.immediate.js":"49tUX","regenerator-runtime/runtime":"dXNgZ","../js/model.js":"Y4A21","./views/recipeView.js":"l60JC","./views/searchView.js":"9OQAM","./views/resultsView.js":"cSbZE","./views/paginationView.js":"6z7bi","./views/bookmarkView.js":"7YaI3","./views/addRecipeView.js":"i6DNj","./config.js":"k5Hzs","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"49tUX":[function(require,module,exports) {
"use strict";
...
parcelHelpers.export(exports, "fetchRecipeDetails", ()=>fetchRecipeDetails);
//1.3.1.1---> when fetching recipe is successful
parcelHelpers.export(exports, "responseSuccessfull", ()=>responseSuccessfull);
parcelHelpers.export(exports, "loadSearchResults", ()=>loadSearchResults);
parcelHelpers.export(exports, "currentPageSearchResults", ()=>currentPageSearchResults);
parcelHelpers.export(exports, "updateServings", ()=>updateServings);
parcelHelpers.export(exports, "toggleBookmarkStatus", ()=>toggleBookmarkStatus);
parcelHelpers.export(exports, "updateBookmarksArray", ()=>updateBookmarksArray);
...
parcelHelpers.export(exports, "clearBMs", ()=>clearBMs);
...
async function fetchRecipeDetails(recipeID) {
    try {
        const jsonFormat = await (0, _helperJs.AJAX)(`${(0, _configJs.API_URL)}/${recipeID}?key=${(0, _configJs.API_KEY)}`);
        responseSuccessfull(jsonFormat);
    } catch (error) {
        throw error;
    }
}
function responseSuccessfull(jsonFormat) {
    //response is successful.
    const { recipe } = jsonFormat.data; //get data from jsonFormat
    (0, _helperJs.renameRecipeDetails)(recipe);
...
const loadSearchResults = async function(query) {
    try {
        if (!query) throw new Error(`Enter some recipe name`);
        ...
        const jsonFormat = await (0, _helperJs.AJAX)(`${(0, _configJs.API_URL)}?search=${query}&key=${(0, _configJs.API_KEY)}`);
        ...
        state.searchResults.totalPages = Math.ceil(state.searchResults.totalResults / (0, _configJs.RES_PER_PAGE));
        if (state.searchResults.totalResults === 0) throw new Error(`We couldnt find that Recipe.Try someother one`);
    } catch (error) {
        throw error; //MODEL-->CONTROLLER--->display in VIEW
    }
};
const currentPageSearchResults = function(page = state.searchResults.currentPageNumber) {
...
const toggleBookmarkStatus = function(status) {
    state.recipe.isBookmarked = !status;
};
const updateBookmarksArray = async function(status) {
    //update array
    if (status) state.bookmarks.push(state.recipe);
    else state.bookmarks = state.bookmarks.filter((recipe)=>{
...
const uploadRecipe = async function(newRecipe) {
    ...
    try {
        ...
        const data = await (0, _helperJs.AJAX)(`${(0, _configJs.API_URL)}?key=${(0, _configJs.API_KEY)}`, recipeObj);
        ...
        (0, _helperJs.renameRecipeDetails)(recipe); //but in this case bookmark will be false as per my logic(thats what we want to call controlBookmark() of controller).
    } catch (error) {
        throw error;
    }
};
...
//contains all the variables(that we will be using through out the project).
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
const API_KEY = `cd355376-a6b2-4306-b38f-e3374c20dde8`;
const API_URL = `https://forkify-api.herokuapp.com/api/v2/recipes`;
const TIMEOUT_SECONDS = 10;
const RES_PER_PAGE = 10;
const MODAL_CLOSE_SEC = 2.5; //try to implement this logic
 /*
export const totalRecipes = state.searchResults.totalResults;
export const totalPages = Math.ceil(state.searchResults.totalResults/10);
*/ 
...
parcelHelpers.export(exports, "renameRecipeDetails", ()=>renameRecipeDetails);
parcelHelpers.export(exports, "createRecipeObject", ()=>createRecipeObject);
parcelHelpers.export(exports, "imageAndTitle", ()=>imageAndTitle);
parcelHelpers.export(exports, "cookingTime", ()=>cookingTime);
parcelHelpers.export(exports, "servings", ()=>servings);
parcelHelpers.export(exports, "userGenerated", ()=>userGenerated);
parcelHelpers.export(exports, "bookMark", ()=>bookMark);
parcelHelpers.export(exports, "recipeIngredients", ()=>recipeIngredients);
parcelHelpers.export(exports, "displayIngredients", ()=>displayIngredients);
parcelHelpers.export(exports, "practicalQuantity", ()=>practicalQuantity);
parcelHelpers.export(exports, "recipeDirections", ()=>recipeDirections);
parcelHelpers.export(exports, "recipeListHTML", ()=>recipeListHTML);
parcelHelpers.export(exports, "friendlyBmMessage", ()=>friendlyBmMessage);
parcelHelpers.export(exports, "pageButtonsHTML", ()=>pageButtonsHTML);
...
const timeout = function(seconds) {
    ...
    return new Promise(function(_, reject) {
        setTimeout(function() {
            reject(new Error(`Request took too long! Timeout after ${seconds} second`)); //manual and meaningful error message
        }, seconds * 1000);
    });
};
...
const renameRecipeDetails = function(recipe) {
    _modelJs.state.recipe = createRecipeObject(recipe);
    //includes() uses strict equality so comparing two objects would return false(even though they are same)
    if (_modelJs.state.bookmarks.some((bookRecipe)=>{
        return bookRecipe.id === recipe.id;
    })) _modelJs.state.recipe.isBookmarked = true;
    else _modelJs.state.recipe.isBookmarked = false;
};
...
const imageAndTitle = function(recipeDetails) {
...
const cookingTime = function(recipeDetails) {
...
const servings = function(recipeDetails) {
    const servingsHTML = `
    <div class="recipe__info">
      <svg class="recipe__info-icon">
...
const userGenerated = function(recipeDetails) {
...
const bookMark = function(recipeDetails) {
    const bookMarkHTML = `<button class="btn--round btn--bookmark">
              <svg class="">
                <use href="${(0, _iconsSvgDefault.default)}#icon-bookmark${recipeDetails.isBookmarked ? "-fill" : ""}"></use>
              </svg>
            </button>`;
    return bookMarkHTML;
};
const recipeIngredients = function(recipeDetails) {
...
const practicalQuantity = function(quantity) {
    if (quantity) return new fraction(quantity).toString();
    else return "";
};
const recipeDirections = function(recipeDetails) {
...
const recipeListHTML = function(recipesArray) {
...
const friendlyBmMessage = function() {
...
const pageButtonsHTML = function(pageNumber) {
    const resultsDetails = _modelJs.state.searchResults;
    //page 1
    if (resultsDetails.currentPageNumber === 1) {
        //page 1,and there are other next pages(no prev pages)
        if (resultsDetails.totalPages > 1) return nextBtnHTML(resultsDetails.currentPageNumber); //curPageNum + 1
        else return ``; //no buttons are displayed
    } else if (resultsDetails.currentPageNumber === resultsDetails.totalPages) return prevBtnHTML(resultsDetails.currentPageNumber); //curPageNum-1
    else return prevBtnHTML(resultsDetails.currentPageNumber) + nextBtnHTML(resultsDetails.currentPageNumber); //curPageNum-1,curPageNUm+1
};
const prevBtnHTML = function(curPageNum) {
    return `<button data-goto="${curPageNum - 1}" class="btn--inline pagination__btn--prev">
            <svg class="search__icon">
...
const nextBtnHTML = function(curPageNum) {
    return `<button data-goto="${curPageNum + 1}" class="btn--inline pagination__btn--next">
            <svg class="search__icon">
              <use href="${0, _iconsSvgDefault.default}#icon-arrow-right"></use>
            </svg>
            <span>Page ${curPageNum + 1}</span>
...
},{"../js/model.js":"Y4A21","../js/config.js":"k5Hzs","../img/icons.svg":"cMpiy","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"cMpiy":[function(require,module,exports) {
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _parentViewJs = require("../views/ParentView.js"); //importing code from parent class to child class
var _parentViewJsDefault = parcelHelpers.interopDefault(_parentViewJs);
var _iconsSvg = require("../../img/icons.svg"); //used for spinner and error/success message
var _iconsSvgDefault = parcelHelpers.interopDefault(_iconsSvg);
var _helperJs = require("../helper.js"); //because i have written many functions of this file in helper.js
class RecipeView extends (0, _parentViewJsDefault.default) {
    ...
    recipeViewListener(handler //handler is function which must be called to handle the event.
    ) {
    ...
    servingsUpdateListener(handler) {
        this._parentElement.addEventListener("click", function(e) {
            ...
            if (newServings > 0) handler(newServings); //call the controlServings()
        });
    }
    ...
    _generateMarkUp() {
        return `
        ${_helperJs.imageAndTitle(this._data)}

          <div class="recipe__details">
            ${_helperJs.cookingTime(this._data)}
            ${_helperJs.servings(this._data)}
            ${_helperJs.userGenerated(this._data)}
            ${_helperJs.bookMark(this._data)}
          </div>

          ${_helperJs.recipeIngredients(this._data)}  

          ${_helperJs.recipeDirections(this._data)}
        `;
...
exports.default = new RecipeView(); //new object is being exported

},{"../views/ParentView.js":"58rNU","../../img/icons.svg":"cMpiy","../helper.js":"lVRAz","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"58rNU":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _iconsSvg = require("../../img/icons.svg"); //used for spinner and error/success message
var _iconsSvgDefault = parcelHelpers.interopDefault(_iconsSvg);
class ParentView {
   ...
   */ render(data, render = true) {
        ...
        if (!render) return markup; //bcoz no parent element to insert the HTML
    ...
    update(data) {
        ...
        newElements.forEach((newEl, i)=>{
            ...
            if (!newEl.isEqualNode(curEl) && curEl) curEl.innerHTML = newEl.innerHTML;
        });
    }
    //1.1-->SPINNER CODE
    startSpinner() {
...
var _helperJs = require("../helper.js"); //because i have written one function of this file in helper.js(recipeListHTML())
class resultsView extends (0, _parentViewJsDefault.default) {
    ...
    _generateMarkUp() {
        return (0, _previewViewJsDefault.default).render(this._data, false); //it should not try to insert HTML,just create markup and return
    }
}
exports.default = new resultsView(); //update parentElement,clear and call methods of ParentView(error/start spinner/stop spinner)

},{"../views/ParentView.js":"58rNU","./previewView.js":"1FDQ6","../helper.js":"lVRAz","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1FDQ6":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _helperJs = require("../helper.js"); //because i have written one function of this file in helper.js(recipeListHTML())
var _parentViewJs = require("./ParentView.js");
var _parentViewJsDefault = parcelHelpers.interopDefault(_parentViewJs);
class previewView extends (0, _parentViewJsDefault.default) {
    ...
    _generateMarkUp() {
        ...
        return _helperJs.recipeListHTML(this._data);
    }
}
exports.default = new previewView();

},{"../helper.js":"lVRAz","./ParentView.js":"58rNU","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"6z7bi":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _parentViewJs = require("../views/ParentView.js"); //importing code from parent class to child class
var _parentViewJsDefault = parcelHelpers.interopDefault(_parentViewJs);
//basically to reuse code inside ParentView class
var _helperJs = require("../helper.js");
class PaginationView extends (0, _parentViewJsDefault.default) {
    ...
    pageBtnClickListener(handler) {
        this._parentElement.addEventListener("click", function(e) {
            ...
            if (!btn) return; //when clicked outside of buttons
    ...
    _generateMarkUp() {
        return _helperJs.pageButtonsHTML(this._data); //sending currentPageNumber
    }
    // 2.8-->to clear buttons when error occurs
    handleError() {
        this._clear();
...
},{"../views/ParentView.js":"58rNU","../helper.js":"lVRAz","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"7YaI3":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _parentViewJs = require("../views/ParentView.js"); //importing code from parent class to child class
var _parentViewJsDefault = parcelHelpers.interopDefault(_parentViewJs);
...
var _previewViewJsDefault = parcelHelpers.interopDefault(_previewViewJs);
var _helperJs = require("../helper.js"); //because i have written one function of this file in helper.js(recipeListHTML())
class bookmarkView extends (0, _parentViewJsDefault.default) {
    //for LHS parent Element is 'results' container.
    ...
    _generateMarkUp() {
        if (this._data.length !== 0) return (0, _previewViewJsDefault.default).render(this._data, false); //it should not try to insert HTML,just create markup and return
        else return _helperJs.friendlyBmMessage();
    }
}
exports.default = new bookmarkView(); //update parentElement,clear and call methods of ParentView(error/start spinner/stop spinner)

},{"../views/ParentView.js":"58rNU","./previewView.js":"1FDQ6","../helper.js":"lVRAz","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"i6DNj":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _parentViewJs = require("../views/ParentView.js"); //importing code from parent class to child class
var _parentViewJsDefault = parcelHelpers.interopDefault(_parentViewJs);
//basically to reuse code inside ParentView class
class addRecipeView extends (0, _parentViewJsDefault.default) {
    ...
    addHandlerUpload(handler) {
        this._parentElement.addEventListener("submit", function(e) {
            ...
            const dataArray = [
                ...new FormData(this)
            ]; //'this' point to HTML element(ie.parentElement or form element)
...
},{"../views/ParentView.js":"58rNU","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["hycaY","aenu9"], "aenu9", "parcelRequire3a11")
